import {
  ...
  updatePostsAnalysis,
} from '@/repositories/facebook/post.repo'
 ...
 */
export const scrapFBPostComments = async (
  days: number,
  categoryId: number
): Promise<{
  ...
  const postsAnalyzed = await analyzePosts(posts, categoryId)
 ...
 */
export const scrapJustFBPosts = async (
  days: number,
  categoryId: number
