import type { FBCommentEntity, FBPostEntity } from '@/interfaces'
import {
  ...
  getCommentsByDate,
} from '@/repositories/facebook/comment.repo'
...
export const syncPostCommentCounts = async (
  ...
  for (const post of posts) {
    const realCount = post.facebook_comment_entity.length
    const analysisCount = post.facebook_comment_analysis.length
...
export const scrapCommentsByDate = async (
  ...
  const posts = (await getPostsByDateWithComments(startDate, endDate)) as unknown as FBPostEntity[]
...
export const analyzeComments = async (): Promise<{ comments: number; status: string }> => {
  const comments = (await getCommentsByDate()) as unknown as FBCommentEntity[]
